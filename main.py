...
# ... 其他 imports ...
import wandb
wandb.init(project="PET-Selection", mode="disabled")
# TensorFlow 会自动被监控（如果使用了 Keras Callback）
...
# ================= 🔧 配置区域 =================
DATA_DIR = r"D:\Processed_Data\NP_data"
    ...
    for e in range(TOTAL_EPISODES):
        ...
        for e in range(TOTAL_EPISODES):
            ...
            for t in range(NUM_BANDS_TO_SELECT):
                # 第一个 Episode 的详细日志保留
                if e == 0:
                    print(f"  [DEBUG] Episode {e + 1} Step {t + 1}/{NUM_BANDS_TO_SELECT}")
            ...
            # -------------------------------------------------------
            # ✅ 正确的位置：在 t 循环结束后（每个 Episode 完成后）打印日志
            # -------------------------------------------------------
            if (e + 1) % 10 == 0:
                print(
                    f"\n🚩 Episode {e + 1}/{TOTAL_EPISODES} | Best Reward: {best_reward:.4f} | Epsilon: {agent.epsilon:.3f}")
                print("📊 当前 3090 Ti 状态:")
                # 使用 os.system 调用时，e 已经有定义了
        ...
        if (e + 1) % 10 == 0:
            print(
                f" 🚩 Episode {e + 1}/{TOTAL_EPISODES} | Best Reward: {best_reward:.4f} | Epsilon: {agent.epsilon:.3f}")
...
if __name__ == "__main__":
    ...
    if not np.isfinite(X_global).all():
        print("⚠️ 警告：数据中仍包含非有限值 (NaN/Inf)，正在强制清洗...")
        # 先处理 NaN 为 0，再裁剪 Inf
        X_global = np.nan_to_num(X_global, nan=0.0, posinf=100.0, neginf=0.0)
        X_global = np.clip(X_global, 0, 100)
    print("✅ 数据清洗完成。")
